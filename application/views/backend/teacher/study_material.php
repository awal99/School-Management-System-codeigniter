    ...
    <tbody>
        ...
        foreach ($study_material_info as $row) { ?>   
            <tr>
                ...
                <td>
                    <?php $name = $this->db->get_where('class' , array('school_id'=>$settings->id,'class_id' => $row['class_id'] ))->row()->name;
                        echo $name;?>
                </td>
                <td>
                    <a href="<?php echo base_url().'uploads/'.$settings->system_name.'/document/'.$row['file_name']; ?>" class="btn btn-blue btn-icon icon-left">
