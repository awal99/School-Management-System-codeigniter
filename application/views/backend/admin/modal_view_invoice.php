        ...
        <table width="100%" border="0">    
            ...
            <tr>
                <td align="left" valign="top">
                    <?php echo $this->db->get_where('s_settings', array('id' => $this->session->userdata('school')))->row()->system_name; ?><br>
                    <?php echo $this->db->get_where('s_settings', array('id' => $this->session->userdata('school')))->row()->address; ?><br>
                    <?php echo $this->db->get_where('s_settings', array('id' => $this->session->userdata('school')))->row()->phone; ?><br>            
                </td>
                <td align="right" valign="top">
                    <?php echo $this->db->get_where('student', array('student_id' => $row['student_id'],'school_id'=>$this->session->userdata('school')))->row()->name; ?><br>
                    <?php 
                        $class_id = $this->db->get_where('student' , array('student_id' => $row['student_id'], 'school_id'=>$this->session->userdata('school')))->row()->class_id;
                        echo get_phrase('class') . ' ' . $this->db->get_where('class', array('class_id' => $class_id, 'school_id'=>$this->session->userdata('school')))->row()->name;
                    ?><br>
                    <?php echo 'roll - ' .  $this->db->get_where('student', array('student_id' => $row['student_id'], 'school_id'=>$this->session->userdata('school')))->row()->roll; ?><br>
                </td>
            </tr>
        </table>
        <hr>
