		...
		<table class="table table-bordered">
			...
			<?php 
				$students = $this->db->get_where('enroll' , array(
					'class_id' => $class_id_from , 'year' => $running_year, 'school_id'=>$this->session->userdata('school')
				))->result_array();
				...
				<tr>
					...
					<button type="button" class="btn btn-default"
						onclick="showAjaxModal('<?php echo base_url();?>index.php?modal/popup/student_promotion_performance/<?php echo urlencode($row['student_id']);?>/<?php echo $class_id_from;?>');">
						<i class="entypo-eye"></i> <?php echo get_phrase('view_academic_performance');?>
					</button>	
					</td>
					<td>
						<?php if($query->num_rows() < 1):?>
							<select class="form-control visible" name="promotion_status_<?php echo $row['student_id'];?>"  id="promotion_status">
...
<script type="text/javascript">
	...
	/* $(document).ready(function() {
		alert("heeeeeeeeeei");
        if($.type($.fn.selectBoxIt) === "function")
		{
			alert("heeeeeeeeee");
